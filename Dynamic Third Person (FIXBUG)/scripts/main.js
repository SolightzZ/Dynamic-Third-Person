...
import { Vector } from "./lib/Vector3";
  ...
  for (let playerData of world.getPlayers()) {
    ...
    if (use_camera[playerData.id] != undefined) {
      ...
      if (speed > 0 || speed_rot > 0.01) {
        ...
        playerData.runCommand(
          "camera @s set dtp:dynamic_third_person ease 0.1 linear pos " +
            offset.location.x +
            " " +
            offset.location.y +
            " " +
    ...
    if (config["dtp:dynamic_fpp"] == "true") {
      ...
      for (let pov of perspective_scan) {
        if (playerData.dimension.getBlockFromRay({ x: pos.x, y: pos.y + 2, z: pos.z }, pov, { maxDistance: 4, includePassableBlocks: false }))
          air_size++;
        if (air_size == 4) break;
      }
    ...
    if (config["dtp:advance_cam_rot"] == "true") {
      ...
      transision_rot[playerData.id] = Math.min(
        Math.max((transision_rot[playerData.id] + perspective * dir_transision * 0.5) * (0.9975 + Math.min(speed * 3, 1) * 0.0025), -15),
        15
      );
    }
    player_rot[playerData.id] = rot;
    ...
    if (player_aim[playerData.id] && config["dtp:scope"] == "1") {
      ...
      camera_option.x =
        (-6 - Math.min(aim_dist[playerData.id], 1) * 2) * (transision_rot[playerData.id] > 0 && config["dtp:advance_cam_rot"] == "true" ? -1 : 1);
      camera_option.y = 1;
    ...
    if (
      player_aim[playerData.id] ||
      playerData.isSneaking ||
      is_riding ||
      is_on_water ||
      config["dtp:free_look"] == "false" ||
      player_fpp[playerData.id] > 0
    ) {
      player_rot_temp[playerData.id] = rot.y;
    ...
    if (config["dtp:vertical_smooth"] == "true") {
      ...
      player_rot_view[playerData.id] =
        player_rot_view[playerData.id] * (1 - Math.min((Math.abs(vel.y) * 0.9 + 0.1) * smooth_y, 1)) +
        loc_res.y * Math.min((Math.abs(vel.y) * 0.9 + 0.1) * smooth_y, 1);
    ...
    if (config["dtp:pbt"] == "true") {
      ...
      let target_pos = {
        x: loc.x + view.x * player_look[playerData.id],
        y: loc.y + view.y * player_look[playerData.id],
        z: loc.z + view.z * player_look[playerData.id],
      };
    ...
    playerData.runCommand(
      "camera @s set dtp:dynamic_third_person ease " +
        camera_option.ease +
        " linear pos " +
        loc_res.x +
        " " +
